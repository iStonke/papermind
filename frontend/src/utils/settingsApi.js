...
export function buildSidebarShowFavoritesPatch(enabled) {
  return { ui: { sidebar_show_favorites: Boolean(enabled) } };
}
...
export function buildOcrScanCleanupPatch(mode) {
  return { ocr: { scan_cleanup: String(mode) } };
}
