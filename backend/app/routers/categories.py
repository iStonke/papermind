...
def create_category(payload: CategoryCreateRequest, db: Session = Depends(get_db)) -> CategoryRead:
    service = CategoryService(db)
...
def update_category(
    category_id: uuid.UUID, payload: CategoryUpdateRequest, db: Session = Depends(get_db)
) -> CategoryRead:
    service = CategoryService(db)
...
def delete_category(category_id: uuid.UUID, db: Session = Depends(get_db)) -> OkResponse:
    service = CategoryService(db)
